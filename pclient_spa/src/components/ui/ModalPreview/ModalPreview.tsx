...
import { HSeparator } from "../../HSepartor/HSeparator";
...
function ModalPreview() {
  const { onClose, open, title, previewFile } = usePreviewStore(
    (state) => state
  );
  ...
  const ShowContent = ({ src }: { src: IPreviewFile | null }) => {
    ...
    if (allowImages.includes(src.type)) {
      return <img src={src.src} alt="" className="h-full object-contain" />;
    }
        ...
        <>
          <div className="flex justify-between items-center">
            ...
            <div
              ...
              onClick={onClose}
            >
              <XCircle size={28} color="white" className="cursor-pointer" />
            </div>
          </div>
